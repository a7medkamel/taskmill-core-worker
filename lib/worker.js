...
Worker.prototype.listen = function(options, cb) {
  ...
  var tunnel_url  = url.format({
    ...
    , hostname : config.get('tunnel.hostname')
  ...
  var tunnel = (port, tunnel_opts) => {
    return Promise.promisify(localtunnel)(port, tunnel_opts);
  };

  var http = () => {
    var ret = express();

    ret.use(responseTime({ header : 'x-tm-response-time-worker' }));
